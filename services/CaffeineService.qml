...
Item {
    ...
    function enable() {
        active = true;
        inhibitProc.command = ["sh", "-c", "systemd-inhibit --what=sleep:idle --who=Quickshell-Caffeine --why='Keep Awake' --mode=block sleep infinity & echo $! > /dev/shm/g_ant_caffeine.pid"];
    ...
    function disable() {
        active = false;
        killProc.command = ["sh", "-c", "if [ -f /dev/shm/g_ant_caffeine.pid ]; then kill $(cat /dev/shm/g_ant_caffeine.pid) 2>/dev/null; rm -f /dev/shm/g_ant_caffeine.pid; fi; pkill -f 'Quickshell-Caffeine' 2>/dev/null || true"];
    ...
    // Clean up inhibit process on destruction to avoid orphans
