...
Item {
    ...
    Connections {
        ...
        function onRawEvent(event) {
            ...
            if (event.name === "monitoradded" || event.name === "monitorremoved") {
                root.applySavedResolutions();
            }
        }
    }

    // Re-apply any saved per-monitor resolutions on hotplug and shortly after boot.
    function applySavedResolutions() {
    ...
    Process {
        id: monitorApplyProc
        command: ["bash", PathSettings.scriptsDir + "/monitors.sh", "apply-saved"]
    }
    ...
    Timer {
        ...
        running: true
        repeat: false
        onTriggered: root.applySavedResolutions()
    }
