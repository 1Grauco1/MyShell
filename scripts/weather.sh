...
fetch_weather() {
    ...
    if [ "$QUERY" = "auto" ]; then
        # Automatic IP-based detection (primary: ipwho.is, HTTPS, no key)
        local geo_data=$(curl -s --max-time 5 "https://ipwho.is/" 2>/dev/null)
        lat=$(echo "$geo_data" | jq -r 'if .success == true then .latitude else empty end' 2>/dev/null)
        lon=$(echo "$geo_data" | jq -r 'if .success == true then .longitude else empty end' 2>/dev/null)
        city=$(echo "$geo_data" | jq -r '.city // empty' 2>/dev/null)

        # Fallback to ipinfo.io
        if [ -z "$lat" ]; then
            local ip_info=$(curl -s --max-time 5 "https://ipinfo.io/json/" 2>/dev/null)
        ...
        if [ -z "$lat" ]; then
            local ip_api_co=$(curl -s --max-time 5 "https://ipapi.co/json/" 2>/dev/null)
            lat=$(echo "$ip_api_co" | jq -r '.latitude // empty' 2>/dev/null)
            lon=$(echo "$ip_api_co" | jq -r '.longitude // empty' 2>/dev/null)
            city=$(echo "$ip_api_co" | jq -r '.city // empty' 2>/dev/null)
        fi
