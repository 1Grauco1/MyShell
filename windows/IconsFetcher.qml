...
QtObject {
    ...
    // Memoized valid-icon results (deterministic per key) to avoid repeated
    // directory walks on frequent events like workspace/focus changes.
    // Capped at 500 entries to prevent unbounded memory growth.
    property var _validIconCache: ({})
    property var _cacheOrder: []
    readonly property int _cacheMaxSize: 500

    function _cacheEvict() {
        while (_cacheOrder.length > _cacheMaxSize) {
            let oldest = _cacheOrder.shift();
            delete _validIconCache[oldest];
        }
    }

    function getValidIcon(appName, desktopEntry, iconName) {
        let key = (appName || "") + "|" + (desktopEntry || "") + "|" + (iconName || "");
        if (key in iconsFetcher._validIconCache) {
            return iconsFetcher._validIconCache[key];
        }
        let result = iconsFetcher._resolveValidIcon(appName, desktopEntry, iconName);
        iconsFetcher._validIconCache[key] = result;
        _cacheOrder.push(key);
        _cacheEvict();
        return result;
    }

    function _resolveValidIcon(appName, desktopEntry, iconName) {
        let raw = (iconName || "").trim();
