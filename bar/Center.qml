...
RowLayout {
    ...
    // Center x of the bar media widget, relative to the bar window origin.
    // Re-evaluates whenever any layout ancestor moves (dynamic center clamp, pill width, etc).
    readonly property real mediaHoverX: {
        if (!mediaSection || !centerContent || !centerPill) return 0;
        return root.x + centerPill.x + centerContent.x + mediaRow.x + mediaSection.x + mediaSection.width / 2;
    }
    ...
    Rectangle {
        ...
        width: DynamicIslandService.active
            ? Theme.scaled(Constants.dynamicIslandActiveWidth)
        ...
        RowLayout {
            ...
            RowLayout {
                ...
                Text {
                    ...
                    font.weight: Font.Normal
                    Layout.alignment: Qt.AlignVCenter
                    text: Qt.formatDateTime(systemClock.date, ClockSettings.use24Hour ? ClockSettings.timeFormat24h : ClockSettings.timeFormat12h)
                }
            }
        }
        ...
        MouseArea {
            ...
            onClicked: (mouse) => {
                ...
                } else if (mouse.button === Qt.RightButton && ClockSettings.showDate) {
                    root.clockShowDate = !root.clockShowDate;
                }
            }
        }
    }
